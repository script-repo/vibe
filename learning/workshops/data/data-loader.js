...
let currentCourseId = 'course-1'; // Default course

/**
 * Gets the current course ID from localStorage or default
 * @returns {string} Course ID
 */
function getCurrentCourseId() {
  return localStorage.getItem('selectedCourse') || 'course-1';
}

/**
 * Sets the current course ID and saves to localStorage
 ...
 * @param {string} courseId - Optional course ID to load (defaults to current)
 * @returns {Promise<Object>} The loaded course data
 */
async function loadCourseData(courseId = null) {
  try {
    // Use provided courseId or get from storage
    if (courseId) {
      currentCourseId = courseId;
    ...
    const response = await fetch(`data/${currentCourseId}.json`);
    if (!response.ok) {
